...
- `src/core/config.ts:13-14` still says the spec mentions zod and that a later task reconciles it; spec §6 was
  reconciled in `8332237`/`86d59b9`, and the spec's only remaining zod mention is "no zod dependency" (`spec:106`).
...
| E6 | Unauthenticated `/healthz` discloses pid, nonce and the absolute binary path | ledger:513-516 | ruled | task-4-review.md:229-234 (launcher liveness probe by design); posture at `0002:389-393` |
