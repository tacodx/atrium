...
Task 10a pre-flight rated the omission BLOCKING. Ruling F (`docs/decisions/0002-slice-rulings.md:469-519`) keeps
...
mutation coverage** (Ruling F `:514-517`). Today `recordFailure` stores `(err as Error)?.message ?? String(err)`
...
kinds, and the pane renders it. Ruling F's revisit trigger (`:519`) — a second provider, and before any provider
...
`open-editor`, `open-terminal` and `open-claude` ship (`src/providers/repos/actions.ts:73-75`).
...
- Launching a browser from `atrium open` is left to a later plan. When it lands, the boot-handoff TTL ruling must be
  re-taken (ADR 0002 `:345-359`); `xdg-open "$(atrium open --print-url)"` already defeats its premise today.
...
- Ruling H's visible error surface is deferred; its revisit (`0002:606`, a second provider) pairs with Ruling F.
...
- A rejected action (a 400) reaches only `console.error` (`web/src/lib/api.ts:86-96`). No ADR text defers it;
  only a source comment says "deferred with Rulings E and H".
...
- Task 4's tests were never run under CPU oversubscription (only CPU starvation, 10a's `podman --cpus`).
...
- The plan cites `scripts/assert-package.ts:108-113` (`:3086`, `:4937`, `:5040`, `:5274`) and `:84-92` (`:5379`);
  the canary is at `:166-178`.
- The plan (`:4935`) and the scoping document (`:134`) give the built stylesheet as 4188 bytes; Task 9 made it
  6004.
...
- `src/core/scheduler.ts:334-335` still suggests async config parsing could make guard B live; `494b1e8` ruled
  that the wrong lever.

**4.12 Operational.**
- CI has never run. `origin/feat/first-light` is at `5f15053`, before `ci.yml` (`f35c2f9`). Pushing is the
  owner's action; the first real run's result is owed to ADR 0003 (e) (`0003:235`).
- The host still holds stale `/tmp/atrium-*` entries from before the P3 fix; removing them is the operator's call.
- Review minors counted but never enumerated (T4 fix review 20, T5 pre-flight 6, T6 review 7, T8 review 11): the
  workflow outputs were not saved, so their dispositions cannot be traced.
...
| A6 | P1: systemd-run present with no user bus makes an exec action a silent no-op | cf2:40-45 | ruled | ADR 0002 Ruling E (`0002:422-467`, `5521e9b`), accepted for this slice. Taken from source, not re-measured (task-8-report.md:261-263) |
...
| A12 | cf2 §4: `resolveGit()` runs a PATH scan on every `runGit` call | cf2:90-92 | ruled | Measured and accepted in `3cf9b99` (25 calls, 12.5 ms at concurrency 8; plan:5780-5782). `rungit.ts` deliberately untouched |
...
| A19 | ADR 0001: a bun 1.4.x upgrade could retire `scripts/gen-assets.ts` once `--asset` is confirmed | 0001:65-69 | ruled | A revisit-on-upgrade trigger. `engines.bun` is still `>=1.3.11` |
...
| B3 | `timedOut` tripwire 2: a bun upgrade making maxBuffer report `killed` would read >16 MB repos as timed out; no maxBuffer test by ruling | ledger:54-58 | ruled | `53200bd`; `0002:302-305`, `src/core/rungit.ts:199-205`. Re-measure ADR row 5 on any bun bump |
| B4 | `timedOut` tripwire 1: an `AbortSignal` on runGit's execFile would make `killed` true on abort | 0002:300-301 | ruled | No `AbortSignal` in `rungit.ts`'s options; warned in its doc comment |
...
| C5 | Guard B (pre-`installSources`) is dead code kept as insurance; it needs its own test if an await ever lands before it | ledger:228-230 | ruled | Documented at `src/core/scheduler.ts:55-68`, `:327-336` (`494b1e8`); record survivors T2-B*, T2-S4c. Residual JSDoc `:334-335` in §4.11 |
...
| D6 | `config: loadConfig()` threading in `src/index.ts` had no runtime observable until Task 9 | ledger:403-404 | closed | `c9863a3`, `a0daceb`. "the assembled server serves the fixture repos over an authenticated /api/state" (asserts `staleDays` 45 from a real config.json); record T2-G RED. The stale text tripwire is J21 |
...
| D8 | `scripts/assert-package.ts` is not run by `bun test`, so its env scoping can regress silently | task-3-report.md:1133-1135 | closed | `87b25c1`, `756aeb5`. "assert:package refuses a /healthz whose pid is not the binary it spawned"; "assert:package reports a spawned binary that exits before it could be tested" |
...
| D10 | `CONFIG_FILENAME` export is unpinned; `describe()`'s per-case wording was unpinned | task-3-report.md:625-631 | open | §4.10 for `CONFIG_FILENAME`. `describe()` is closed by `cc51b2d`: "a non-object top level throws ConfigError naming what it got instead" |
...
| E4 | X14 can write a live credential into `~/.config/atrium`; run it scoped to `test/paths.test.ts` | ledger:502-504 | ruled | plan:1890-1896. The 10b replay ran it on the full suite, safely here because that directory is absent; the hazard is documented, not structural |
...
| E6 | Unauthenticated `/healthz` discloses pid, nonce and the absolute binary path | ledger:513-516 | ruled | task-4-review.md:229-234 (launcher liveness probe by design); posture at `0002:382-386` |
| E7 | Token comparisons are not constant-time; the first recorded reason was wrong (loopback is interface-scoped); timing never measured | ledger:517-520 | ruled | `44aac81`, `f68a5d1`: `0002:386-420`, "an analytical argument, not a measurement", revisit if off-host or a second `mintHandoff` caller |
...
| E9 | I1: in-process `startServer` calls in `test/serve.test.ts` had no `env`, so `bun test` overwrote then deleted a running atrium's files; the "dead credential left behind" variant was never reproduced | ledger:548-555 | open | §4.10. Fixed in `abd2830` (all 29 in-process calls pass `env` at `4f7956c`), but no test pins it: the structural spawn test excludes in-process calls (`test/verify-gate.test.ts:152-157`) |
...
| E11 | I4: the 7-day TTL premise ("never in a URL") is designed to become false; the ruling needed a revisit trigger | ledger:566-568 | ruled | `4441844`, `acdfa8e`: trigger at `0002:345-359`. Source-comment half is E21 |
...
| E18 | The T4 tests were never flake-checked under CPU oversubscription | ledger:576-577 | open | §4.10. 10a's 13 runs under `podman --cpus 4/2/1` (287/0, ledger:1299-1302) measure starvation, not the 64-busy-loop shape T2/T3 used |
...
| E23 | The M1 guard's rationale was inverted (`not.toContain('')` throws on bun 1.3.11), and assertions in `finally` masked the real failure | ledger:731-733 | closed | `f0eb37b`. Same test; record T4-R2 and T4-M1-guard RED |
| E24 | The `open` success path piped stderr and never read it | ledger:727-730 | closed | `34cdb95`. Same test (openErr assertion); record T4-T1 RED |
...
| E28 | ADR-lens wording faults in the boot-handoff section (A1-A8) | ledger:734-739 | ruled | Text-only, `f68a5d1`, `acdfa8e` (`0002:328-329`, `:353-359`, `:400-420`) |
| E29 | Review minors counted but never enumerated (T4 fix review 20, T5 pre-flight 6, T6 review 7, T8 review 11) | ledger:692-693, :721, :877, :1016 | open | §4.12. The workflow outputs were not saved; only counts survive. The T4 fix2 brief accounts for about 18 of 20. Cannot be resolved from the tree |
| E30 | No rate limiting on `POST /api/session` | plan:1921-1922 | ruled | Out of scope by plan:1921-1922; 256-bit tokens make brute force irrelevant (task-4-report.md:616-617) |
| E31 | One boot handoff per start means one browser profile can redeem | 0002:361-370 | ruled | ADR 0002 "Accepted residual" |
| E32 | A stale `handoff.json`/`endpoint.json` prints a well-formed URL that silently fails | 0002:372-376 | open | §4.7. `src/core/paths.ts:37-39`: "documented future work"; no doctor command |
...
| E34 | `BOOT_HANDOFF_TTL_MS` is pinned only as ≥24 h; the mint sweep never fires in production | 0002:340-343 | ruled | "the boot handoff TTL is long, per the recorded ruling" is labelled a tripwire; the sweep is exercised by "minting sweeps handoffs that expired before the new mint" |
...
| F3 | Re-running M6 expects TS2322/TS2345/TS1360, not TS2741 alone; A2 typechecks between edits | ledger:698-701 | ruled | Delivered as record rows T5-M6, T5-M6-routes, T5-A2 (all RED_TYPECHECK). Typecheck rows have no test title |
...
| F7 | Ruling F's gate must be written into the Plan 3 carry-forward as a precondition on the ordering (rated BLOCKING) | ledger:1082-1084 | ruled | Written in §3 of this document, quoting `0002:500-503` |
...
| F14 | L2-5: `toClient` returning `undefined` looks like "never succeeded" on the wire | ledger:804 | open | §4.8. Unreachable for `reposToClient` (`index.ts:608`); the pane treats `undefined` as loading; nobody recorded a disposition |
...
| G4 | R-B: `socket.ts` said `onAuthFailure` re-reads the fragment; the shipped `main.tsx` clears and re-renders | ledger:907-910 | ruled | ADR 0002 Ruling G (`0002:521-565`, `a138b85`); comment corrected in `5a6971c`. Wiring has no test (`main.tsx` is not importable under bun test). Citation drift `:48`→`:50` in §4.11 |
| G5 | Ruling H: the client drops the wire `error` frame; the visible surface is deferred | ledger:1180-1182 | ruled | `0002:567-609`. Pinned by "an error frame is dropped before and after the snapshot: no state change, no notification" (`476a386`, `345e01e`, `1c54ece`). Revisit with a second provider, paired with F (§4.8) |
...
| G9 | Nothing uses a real browser: CSP-vs-Tailwind, localStorage `SecurityError` and live WS behaviour are unobserved | task-6-report.md:330-333 | ruled | Structural: ADR 0002 Ruling D Property 2 and ADR 0003 (d) (`0003:223-225`) |
...
| H3 | The in-root symlink test was vacuous against a symlink-following walker (15th vacuity) | ledger:961-963 | closed | `4d42cfd`. "a symlink to a repo inside the root does not produce a second entry"; "a symlink pointing out of the scanned root never reaches Data"; record T7-R1 RED |
...
| I3 | F3: an empty `log` stdout gave `lastCommitAt` 0 | ledger:1035-1036 | ruled | Guard in `43152f5`; unreachable on git 2.55 (plan:5841-5846) |
...
| I6 | Ruling I limits 1-5 (other-name frontends, wrappers such as `env`/`sh -c`, aliases and links, shipped defaults, unscanned args); cf2 §4's name-check warning | cf2:85-89; 0002:665-681 | ruled | Recorded limits; revisit if a realpath layer is proposed (`0002:692`) |
| I7 | Ruling I limit 6: `.exe` unmatched, and narrowing the suffix class is caught by nothing | 0002:682-683 | open | §4.10. "Minor, unpinned"; no test or record row targets it |
...
| I9 | P10: the `actions.ts:25` comment said a spawned child's cwd is `/` | ledger:1525-1526 | ruled | Comment-only in `2979b62`; side correction at `0002:685-690` |
...
| I14 | The 10-minute discovery run can overlap a 30-second metadata pass | plan:5800-5805 | ruled | Reasoned, not measured; carry-over on rebuild heals it |
...
| J2 | Carried constraints: type-only wire imports, Tailwind only, a literal `p-4`, no wrapper hook | ledger:1058-1063 | closed | `a0daceb`, `26dd62e`. "the pane source contains no injection sink and no value import from src/"; "main.tsx still contains the literal p-4 the packaging gate depends on". No-wrapper-hook by inspection only |
...
| J7 | A rejected action (400) shows nothing in the pane; only `console.error` | ledger:1168-1170 | open | §4.9. `api.ts:74-96` says "deferred with Rulings E and H", but neither ADR ruling names it, and `test/repos-pane.test.ts:466` still calls it T10's |
...
| J28 | The plan's "4188 bytes" built stylesheet is stale (6004 after T9) | ledger:1136-1137 | open | §4.11. plan:4935, scoping:134; outside 10c D3 |
...
| J34 | The plan cites stale `scripts/assert-package.ts` lines (`:108-113`, `:84-92`) | ledger:1270-1272 | open | §4.11. ADR 0003 avoided them; the plan text is outside 10c D3 |

### K. Task 10 (10a, closeout, 10b, 10c)

| # | Item | First carried | Disposition | Evidence |
|---|---|---|---|---|
| K1 | The release gate passed vacuously: `assert:package` never checked `/healthz` was its own child; a dev server on 7373 made it pass | ledger:1312-1317; plan:5211-5215 | closed | `c8dd615`, `87b25c1`, `37d4f72`, `756aeb5`. "assert:package refuses a /healthz whose pid is not the binary it spawned"; "assert:package fails the measured scenario: a stub that exits ${exitCode} while a convincing listener answers" |
...
| K6 | P-3: re-measure every reference number instead of copying them | ledger:1273-1274 | ruled | `0003:161-186`, `:246-257` (`b5eb99d`) |
| K7 | CI has never executed; ADR 0003 (e) owes the first real run's result | ledger:1245-1246 | open | §4.12. `origin/feat/first-light` = `5f15053`, before `f35c2f9`; pushing is the owner's action |
...
| K12 | Closeout limitations F2-F5: `:/x` omitted, lexical per-file binding scan, looser-than-tsc resolver, and the F5 `/tmp` scan missing a variable-bound literal, `/var/tmp`, and `mkdirSync` | ledger:1573-1575 | ruled | Terminal condition "documented, not chased" (task-10-closeout-fix-report.md:127-143). In-tree notes at `test/repos-discovery.test.ts:402-403`, `test/launcher-pin.test.ts:18-33`, `test/web-lib-imports.test.ts:32-33`; the F5 misses and F3's cross-file forwarding case are tracked only here |
...
| K15 | Host still holds stale `/tmp/atrium-*` entries (6152 at T8, 1276 at the closeout) | plan:5872-5873 | open | §4.12. Operator's call; deliberately untouched |
...
| K17 | Two corrections for 10b: cross-task row 9 is unobservable (run T6-M22 expecting four reds); row 6's parenthetical is stale | ledger:1335-1338 | closed | Record TX-9 duplicates T6-M22, RED on "server.stop() stops the scheduler" among four; TX-6 notes carry the correction |
